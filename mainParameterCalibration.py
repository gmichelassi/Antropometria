...
def __testToRun():
    ...
    classifiers = [rf('randomforestclassifier'), svm('svc'), knn('kneighborsclassifier'), nnn('mlpclassifier'),
                   nb('gaussiannb')]
...
def __errorEstimation(lib='dlibHOG', dataset='distances_all_px_eu', classes=None, model=None, parameters=None,
                      reduction='None', filtro=0.0, amostragem=None, min_max=False):
    ...
    try:
        X, y, synthetic_X, synthetic_y = run_pre_processing(lib=lib, dataset=dataset, classes=classes,
                                                            reduction=reduction, filtro=filtro, amostragem=amostragem,
                                                            split_synthetic=True, min_max=min_max, verbose=False)
    ...
    mean_results = mean_scores({'accuracy': accuracy, 'precision_micro': precision_micro, 'recall_micro': recall_micro,
                                'f1_micro': f1_micro, 'precision_macro': precision_macro, 'recall_macro': recall_macro,
                                'f1_macro': f1_macro, 'time': tempo})
            ...
            for filtro in filtros:
                for min_max in min_maxs:
                    for amostragem in amostragens:
                        ...
                        log.info("Running test for [lib: %s, classifier: %s, reduction: %s, filter: %s, min_max: %s, "
                                 "sampling: %s]", lib, classifier.name, reduction, filtro, min_max, amostragem)

                        try:
                            X, y, synthetic_X, synthetic_y = run_pre_processing(lib=lib, dataset=dataset,
                                                                                classes=classes, reduction=reduction,
                                                                                filtro=filtro, amostragem=amostragem,
                                                                                split_synthetic=False, min_max=min_max)
                        except RuntimeError as re:
                            log.info("It was not possible run test for [classifier: %s, reduction: %s, filter: %s, "
                                     "min_max: %s, sampling: %s]", classifier.name, reduction, filtro, min_max,
                                     amostragem)
                            log.info("Error: " + str(re))
                            continue
                        ...
                        if grid_results is not None:
                            log.info("Best result presented accuracy %.2f%% for test [classifier: %s, reduction: %s, "
                                     "filter: %s, min_max: %s, sampling: %s]", grid_results.best_score_ * 100,
                                     classifier.name, reduction, filtro, min_max, amostragem)
                            ...
                            try:
                                errResults = __errorEstimation(lib=lib, dataset=dataset, model=classifier,
                                                               parameters=grid_results.best_params_,
                                                               reduction=reduction, filtro=filtro, amostragem=amostragem,
                                                               min_max=min_max)
                            except KeyError as ke:
                                errResults = {'accuracy': '', 'IC': '', 'precision': '', 'recall': '', 'f1score': '',
                                              'time': ''}
                            ...
                            with open(f"./output/GridSearch/{lib}_best_results.csv", "a") as csvfile:
                                writer = csv.DictWriter(csvfile, fieldnames=['biblioteca', 'classifier', 'reduction',
                                                                             'filtro', 'min_max', 'par_amostragem',
                                                                             'par_accuracy', 'parameters',
                                                                             'err_accuracy', 'err_IC',
                                                                             'err_precision_micro', 'err_recall_micro',
                                                                             'err_f1score_micro', 'err_precision_macro',
                                                                             'err_recall_macro', 'err_f1score_macro',
                                                                             'err_time'])
                                results = {
                                    'biblioteca': lib,
                            ...
                            path_results = './output/GridSearch/results_{0}_{1}_{2}_{3}_{4}_{5}_{6}.csv'\
                                .format(lib, dataset, classifier.name, reduction, filtro, min_max, amostragem)
