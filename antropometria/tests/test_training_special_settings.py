...
class TestTrainingSpecialSettings:
    def test_rf_stops_running(self):
        ...
        assert stop_running_rf(
            is_random_forest_done=True, classifier_name='AnotherClassifier', reduction='AnyReduction')

    def test_skips_current_test(self):
        assert not skip_current_test(is_random_forest_done=False, classifier=RANDOM_FOREST, reduction='Any')
        assert skip_current_test(is_random_forest_done=True, classifier=RANDOM_FOREST, reduction='Any')
        assert skip_current_test(is_random_forest_done=False, classifier=OTHER_CLASSIFIER, reduction=None)
