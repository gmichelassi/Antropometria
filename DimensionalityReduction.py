...
def load(lib, dataset, filtro, min_max, verbose):
    if dataset == 'distances_all_px_eu':
        if lib == 'ratio':
            X, y = asd.load_data(lib=lib, dataset=dataset, classes=['controles', 'casos'], ratio=True, verbose=verbose)
        else:
            X, y = asd.load_data(lib=lib, dataset=dataset, classes=['controles', 'casos'], ratio=False, verbose=verbose)
    else:
